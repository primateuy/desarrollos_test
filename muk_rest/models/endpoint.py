...
class Endpoint(models.Model):
    ...
    @api.model
    def _get_eval_context(self, request, model):
        return {
            'time': time,
            'datetime': datetime,
            'dateutil': dateutil,
            ...
            'make_json_response': request.make_json_response,
            'make_response': request.make_response, 
            'responses': responses,
    ...
    def _evaluate_domain(self, request, user):
    ...
    def _evaluate_action(self, request, user):
        active_id = common.parse_value(request.params.get('id'))
        active_ids = common.parse_value(request.params.get('ids'), [])
        model_with_user = self.env[self.model_id.model].with_user(user)
        model = model_with_user.sudo() if self.eval_sudo else model_with_user
    ...
    def _evaluate_code(self, request, user):
        model_with_user = self.env[self.model_id.model].with_user(user)
        model = model_with_user.sudo() if self.eval_sudo else model_with_user
        eval_context = self._get_eval_context(request, model)
        ...
        if eval_context.get('result', False):
            ...
            return request.make_json_response(eval_context['content'])
    ...
    def evaluate(self, request, user):
        ...
        if hasattr(self, '_evaluate_{}'.format(self.state)):
            return getattr(self, '_evaluate_{}'.format(self.state))(request, user)
