...
class pos_coupons_setting(models.Model):
	...
	def _check_amount(self):
		if self.min_coupan_value >= self.max_coupan_value:
			raise exceptions.ValidationError(_("Minimum amount must be less than maximum amount"))
		if self.max_coupan_value <= self.min_coupan_value:
			raise exceptions.ValidationError(_("Maximum amount must be greater than minimum amount"))
	...
	def create(self, vals_list):
		...
		if self.search_count([('active','=',True)]) > 0:
			raise exceptions.ValidationError(_('Allows only one activated Configuration of POS'))
		else:
			return super(pos_coupons_setting,self).create(vals_list)
