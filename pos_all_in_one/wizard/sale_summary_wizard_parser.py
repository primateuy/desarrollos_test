...
class SaleSummaryReport(models.AbstractModel):
	...
	def _get_report_values(self, docids, data=None):
		...
		for u in sale_summary_rec.res_user_ids:			
			...
			if st_line_ids:
				self.env.cr.execute("""
					SELECT COALESCE(ppm.name->>%s, ppm.name->>'en_US') d_name, sum(amount) total
			...
			if product_ids:
				self.env.cr.execute("""
					SELECT COALESCE(pc.name->>%s, pc.name->>'en_US') c_name, sum(qty * price_unit) c_total
					FROM pos_order_line AS pol,
						 pos_category AS pc,
						 product_product AS product,
						 product_template AS templ
					WHERE pol.product_id = product.id
						AND product.bi_pos_reports_catrgory = pc.id
