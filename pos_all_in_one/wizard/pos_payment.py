	...
	def print_excel_report(self):
			...
			for order in pos_orders:
				...
				for line in order.lines:
					...
					price_incl_tax = round(line.price_subtotal_incl / line.qty, 2)
					key = (line.product_id, line.price_unit, disc , price_incl_tax )
			...
			data = {'products': sorted([{
				...
				'quantity': qty,
				'price_unit': price_incl_tax,
				'discount': discount,
				'uom': product.uom_id.name,
			} for (product, price_unit, discount ,price_incl_tax), qty in products_sold.items()], key=lambda l: l['product_name'])}
			
			st_line_ids = self.env["pos.payment"].search([('pos_order_id', 'in', pos_orders.ids)]).ids			
			if st_line_ids:
				self.env.cr.execute("""
					SELECT COALESCE(ppm.name->>%s, ppm.name->>'en_US') s_name, sum(amount) total
			...
			for product_details in data.get('products') : 
				...
				worksheet.write(r, 3, str(round(product_details.get('quantity') * product_details.get('price_unit'),2)), num_style)
				p_qty =  p_qty   + product_details.get('quantity') 
				total += (product_details.get("quantity") * product_details.get("price_unit"))		 
			...
			for payment in payments:
				r = r + 1				
				worksheet.write(r, 0, payment.get('s_name'))
				worksheet.write(r, 3, str(payment.get('total',2)), num_style)
			...
			fp = io.BytesIO()
			workbook.save(fp)
			export_id = self.env['sale.excel.report'].create({'excel_file': base64.encodebytes(fp.getvalue()), 'file_name': filename})
			fp.close()
			return {
				'view_mode': 'form',
				'res_id': export_id.id,
