...
class PosXReport(models.TransientModel):
	...
	def generate_x_report(self):
		...
		return self.env.ref('pos_all_in_one.action_x_report_print').report_action([], data=data)

class OpenSessionReport(models.AbstractModel):

	_name = 'report.pos_all_in_one.report_open_session'
	...
	def get_sale_details(self,sessions=False):
		...
		for order in orders:
			if order.is_partial:
				partial_amount = partial_amount + (order.amount_total - sum(order.payment_ids.mapped("amount")))
